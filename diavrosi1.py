...
human_factors = {'Ουδέτερη': 1.0, 'Αρνητική (καταστροφική χρήση)': 1.3, 'Θετική Περιβαλλοντική Διαχείριση': 0.7}
...
with col2:
    ...
    ανθρωπινη_παρεμβαση = st.selectbox("Ανθρώπινος Παράγοντας", list(human_factors.keys()))
...
with st.expander("📐 Τι είναι η Γωνία Πρόσκρουσης Κυμάτων;"):
    ...
    ax_angle.annotate("90°", xy=(3.1, 0.5), xytext=(3.8, 0.5),
    ...
    st.markdown("""
    - 🟩 **0°** → Κύματα **κάθετα** στην ακτή → **μέγιστη διάβρωση**
    - 🟧 **45°** → Κύματα **λοξά** → διάβρωση & μεταφορά υλικών
    - 🟥 **90°** → Κύματα **παράλληλα** στην ακτή → **ελάχιστη κάθετη διάβρωση**
    """)
...
H = human_factors[ανθρωπινη_παρεμβαση]

erosion_rate = K * ύψος_κύματος * συχνότητα * abs(np.cos(θ_rad)) * P * H
...
with st.expander("🌿 Πώς βοηθά η Βλάστηση;"):
    ...
    - **Μειώνει** την απώλεια άμμου και υλικών από τον άνεμο
    - Στην προσομοίωση, μειώνει τη διάβρωση κατά **30%**
    """)

# Επεξήγηση Ανθρώπινου Παράγοντα
with st.expander("🏗 Ανθρώπινος Παράγοντας και Διάβρωση"):
    st.markdown("""
    Ο τρόπος που οι άνθρωποι επεμβαίνουν στην παραλία **επηρεάζει σημαντικά** τη διάβρωση:

    - 🚧 **Αρνητικές επεμβάσεις** όπως αυθαίρετες κατασκευές, αποψίλωση, ή βαριά χρήση επιδεινώνουν το φαινόμενο.
    - 🌱 **Θετικές ενέργειες** όπως βλάστηση, σωστός σχεδιασμός και ενημέρωση **μειώνουν τη διάβρωση**.
    """)

# Επεξήγηση Θετικής Περιβαλλοντικής Διαχείρισης
with st.expander("🌍 Παραδείγματα Θετικής Περιβαλλοντικής Διαχείρισης"):
    data = {
        "Ενέργεια": ["🌿 Φύτευση βλάστησης", "🧱 Φυσική προστασία", "🧼 Αποφυγή ρύπανσης", "🏖 Έλεγχος πρόσβασης", "📚 Εκπαίδευση"],
        "Τι κάνει": [
            "Ρίζες που συγκρατούν το έδαφος",
            "Χρήση φυσικών υλικών για απόσβεση κυμάτων",
            "Προστασία από χημικά και απορρίμματα",
            "Περιορισμός σε ευαίσθητες περιοχές",
            "Ενημέρωση μαθητών και επισκεπτών"
        ],
        "Πώς βοηθά": [
            "Μειώνει τη διάβρωση",
            "Απορροφά την ενέργεια των κυμάτων",
            "Διατηρεί την υγεία του οικοσυστήματος",
            "Μειώνει την πίεση στην παραλία",
            "Ενισχύει υπεύθυνη στάση"
        ]
    }
    df_δράσεις = pd.DataFrame(data)
    st.dataframe(df_δράσεις, use_container_width=True)
